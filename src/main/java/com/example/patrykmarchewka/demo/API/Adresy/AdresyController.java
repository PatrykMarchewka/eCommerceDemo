...
public class AdresyController {
    ...
    public ResponseEntity<OdpowiedzAPI<List<AdresyDTO>>> getAllAdresy(@PathVariable Long uzytkownikID){
        List<AdresyDTO> dtoList = adresyService.getAllAdresy(uzytkownicyService.getUzytkownikByID(uzytkownikID)).stream().map(item -> new AdresyDTO(item)).toList();
    ...
    public ResponseEntity<OdpowiedzAPI<AdresyDTO>> getAdresyByID(@PathVariable Long uzytkownikID, @PathVariable Long adresyID){
        return ResponseEntity.ok(new OdpowiedzAPI<>("Informacje o zapisanym adresie",new AdresyDTO(adresyService.getByKlientAndID(uzytkownicyService.getUzytkownikByID(uzytkownikID),adresyID ))));
    }
    ...
    public ResponseEntity<OdpowiedzAPI<AdresyDTO>> createAdresy(Authentication authentication,@RequestBody AdresyRequestBody body){
    ...
    public ResponseEntity<OdpowiedzAPI<AdresyDTO>> getAdresyByID(Authentication authentication, @PathVariable Long adresyID){
        return ResponseEntity.ok(new OdpowiedzAPI<>("Informacje o zapisanym adresie", new AdresyDTO(adresyService.getByKlientAndID(uzytkownicyService.getUzytkownik(authentication),adresyID))));
    }

    @PutMapping("/me/adresy/{adresyID}")
    public ResponseEntity<OdpowiedzAPI<AdresyDTO>> putAdresyByID(Authentication authentication, @PathVariable Long adresyID, @RequestBody AdresyRequestBody body){
    ...
    public ResponseEntity<OdpowiedzAPI<AdresyDTO>> patchAdresyByID(Authentication authentication, @PathVariable Long adresyID, @RequestBody AdresyRequestBody body){
        Uzytkownicy uzytkownik = uzytkownicyService.getUzytkownik(authentication);
        Adresy adres = adresyService.getByKlientAndID(uzytkownik,adresyID);
