package com.example.patrykmarchewka.demo.API.Uzytkownicy;
import com.example.patrykmarchewka.demo.API.Adresy.Adresy;
import com.example.patrykmarchewka.demo.API.Adresy.AdresyDTO;
import com.example.patrykmarchewka.demo.API.RoleUzytkownikow;
import com.fasterxml.jackson.annotation.JsonProperty;

import java.util.ArrayList;
import java.util.List;

public class UzytkownicyDTO {
    @JsonProperty("Unikalny identyfikator ID")
    private Long ID;
    @JsonProperty("Kod Vat ID")
    private String vatID;
    @JsonProperty("Imie")
    private String imie;
    @JsonProperty("Nazwisko")
    private String nazwisko;
    @JsonProperty("Email")
    private String email;
    @JsonProperty("Haslo")
    private String haslo;
    @JsonProperty("Adresy")
    private List<AdresyDTO> adres = new ArrayList<>();
    @JsonProperty("Numer telefonu")
    private String telefon;
    @JsonProperty("Rola uzytkownika")
    private RoleUzytkownikow rola;


    public UzytkownicyDTO(Uzytkownicy uzytkownik){
        this.ID = uzytkownik.getID();
        this.vatID = uzytkownik.getVatID();
        this.imie = uzytkownik.getImie();
        this.nazwisko = uzytkownik.getNazwisko();
        this.email = uzytkownik.getEmail();
        this.haslo = uzytkownik.getHaslo();
        for (Adresy adres : uzytkownik.getAdres()){
            this.adres.add(new AdresyDTO(adres));
        }
        this.telefon = uzytkownik.getTelefon();
        this.rola = uzytkownik.getRola();
    }

    public UzytkownicyDTO(){}


    public String getVatID() {
        return vatID;
    }

    public void setVatID(String vatID) {
        this.vatID = vatID;
    }

    public String getImie() {
        return imie;
    }

    public void setImie(String imie) {
        this.imie = imie;
    }

    public String getNazwisko() {
        return nazwisko;
    }

    public void setNazwisko(String nazwisko) {
        this.nazwisko = nazwisko;
    }

    public String getEmail() {
        return email;
    }

    public void setEmail(String email) {
        this.email = email;
    }

    public String getHaslo() {
        return haslo;
    }

    public void setHaslo(String haslo) {
        this.haslo = haslo;
    }

    public List<AdresyDTO> getAdres() {
        return adres;
    }

    public void setAdres(List<AdresyDTO> adres) {
        this.adres = adres;
    }

    public String getTelefon() {
        return telefon;
    }

    public void setTelefon(String telefon) {
        this.telefon = telefon;
    }

    public RoleUzytkownikow getRola() {
        return rola;
    }

    public void setRola(RoleUzytkownikow rola) {
        this.rola = rola;
    }

    public Long getID() {
        return ID;
    }
}
